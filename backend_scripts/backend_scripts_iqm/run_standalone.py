...
for f_idx, qasm_file in enumerate(qasm_files):
    ...
    try:
        ...
        try:
            ...
            plt.savefig(
                os.path.join(output_dir, f"{base_name}_original_circuit.png"),
                dpi=300,
                bbox_inches="tight",
            )
            plt.close(fig)
        ...
        try:
            ...
            plt.savefig(
                os.path.join(output_dir, f"{base_name}_transpiled_circuit.png"),
                dpi=300,
                bbox_inches="tight",
            )
            plt.close(fig)
        ...
        transpiled_qc_filename = os.path.join(
            output_dir, f"{base_name}_transpiled.qasm2"
        )
        ...
        if results_dict:
            try:
                ...
                fig, ax = plt.subplots(
                    figsize=(max(6, len(states) * 0.5), 6)
                )  # dynamic width
