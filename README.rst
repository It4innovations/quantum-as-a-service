...
**Prerequisites:** The `py4lexis` package (for LEXIS authentication) must be installed first.
...
**QProvider** The main entry point for quantum resource access through LEXIS.
...
* ``resource_name`` (str): Accounting string (typically ``"EQE1-CZ-P0001"``). Depending on the selected resource name, a backend for submission will be selected.

**QBackend** Represents quantum hardware interface with capabilities for circuit execution.
...
**QJob** Manages quantum job execution and result retrieval.
...
2. ``run_init.sh`` sets up environment or returns particular system information
