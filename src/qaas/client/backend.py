...
class QBackend:
    ...
    def _get_backend_from_heappe(self):
        ...
        init_job_data = {
            ...
            "environment_variables": [
                EnvironmentVariableExt(name="Q_COMMAND", value="backend_init")
            ],
        }
        q_arg_value = ""
        ...
        if q_arg_value != "":
            init_job_data["environment_variables"].append(
                EnvironmentVariableExt(name="Q_OPTIONAL_ARG", value=q_arg_value)
            )
        ...
        results = self._qclient.get_job_results(
            self.init_job_id,
            [f"/{self.init_job_id}/{self.init_task_ids[0]}/backend.pkl"],
            use_dill=[False],
    ...
    def run(
        self,
        run_input: QuantumCircuit | list[QuantumCircuit] | str | list[str],
        shots=1000,
        ...
        **run_options,
        ...
        job_name_core = (
            getattr(run_input, "name", "circuits")
        ...
        if self._qclient.provider_token:
            ...
            job_data["environment_variables"] = EnvironmentVariableExt(
                name=token_var_name, value=self._qclient.provider_token
            )
...
class QJob:
    ...
    def result(
        ...
        heappe_results = self._qclient.get_job_results(
            self.job_id,
            circuit_job_files if self._type == "circuit" else pulla_job_files,
            use_dill=circuit_job_use_dill
        ...
        if "job" in heappe_results and heappe_results["job"]:
            ...
            if self._type == "circuit":
                self._transpiled_circuits = heappe_results.get("transpiled_circuits")
            instance_update_ended = time.time()

            self.qaas_instance_update_runtime = (
                instance_update_ended - instance_update_started
            )
            ...
            log.debug(
                "Qprovider instances updated (IQMjob) in: %f s",
                self.qaas_instance_update_runtime,
            )
                ...
                if hasattr(self.remote_job, "data") and self.remote_job.data.messages:
                    log.debug(
                        ...
                        "\n".join(
                            ...
                            for msg in self.remote_job.data.messages
                        ),
                    )
